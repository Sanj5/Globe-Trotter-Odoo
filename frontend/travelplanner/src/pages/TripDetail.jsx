...
const TripDetail = () => {
  ...
  // Calculate expected expense from activities
  const calculateExpectedExpense = () => {
    let total = 0;
    
    // Sum up costs from user-added activities
    trip.stops?.forEach((stop) => {
      stop.activities?.forEach((act) => {
        const cost = act.estimated_cost_override ?? act.activity?.estimated_cost ?? 0;
        total += Number(cost) || 0;
      });
    });
    
    // Add AI itinerary costs if available
    if (aiItinerary?.days) {
      aiItinerary.days.forEach((day) => {
        ...
        // Transport
        if (day.transport_to_next_day?.modes) {
          day.transport_to_next_day.modes.forEach((mode) => {
            total += Number(mode.cost) || 0;
          });
        }
        
        // Accommodation
        if (day.accommodation?.cost) {
          total += Number(day.accommodation.cost) || 0;
        }
        
        // Meals
        day.meals?.forEach((meal) => {
          total += Number(meal.cost) || 0;
        });
      });
    }
    
    // Use AI budget breakdown if available and no manual calculations
    if (total === 0 && aiItinerary?.budget_breakdown) {
      total = Object.values(aiItinerary.budget_breakdown).reduce((sum, val) => sum + (Number(val) || 0), 0);
    }
    
    return total;
  };

  const expectedExpense = calculateExpectedExpense();
      ...
      <div className="trip-detail-header">
        <div>
          ...
          <p className="trip-dates">
            ...
            {expectedExpense > 0 && <span> • Expected Expense: <strong>₹{expectedExpense.toLocaleString('en-IN')}</strong></span>}
          </p>
